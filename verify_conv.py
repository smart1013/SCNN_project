...
# Output 3x10x10
OA = [[[0.0 for _ in range(10)] for _ in range(10)] for _ in range(3)]

for k in range(3):
    for c in range(3):
        # 3x3 conv
        kernel = FWs[k][c]
        for h_out in range(10):
            for w_out in range(10):
                # Stride 1, Pad 1
                # Center of kernel corresponds to h_out, w_out in input (roughly)
